...
#ifndef KVDB_HASH_DICTIONARY_H
#define KVDB_HASH_DICTIONARY_H
...
private:
    ...
    void initialize(size_type new_capacity) {
        size_type bucket_capacity = detail::round_up_pow2(new_capacity);
        ...
        // Preserve the bucket list size.
        buckets_List_.clear();
        buckets_List_.reserve(4);
    ...
    inline size_type calc_capacity(size_type new_capacity) {
        ...
        new_capacity = detail::round_up_pow2(new_capacity);
        return new_capacity;
    }
...
public:
    void dump() {
        printf("KVCache::BasicDictionary<K, V>::dump()\n\n");
    }
    ...
    iterator find(const key_type & key) {
        if (likely(this->buckets() != nullptr)) {
            ...
            index_type index = this->hasher_.index_of(hash, this->bucket_mask_);
    ...
    inline iterator find_before(const key_type & key, entry_type *& before_out, size_type & index) {
        hash_type hash = this->hasher_.hash_code(key);
        index = this->hasher_.index_of(hash, this->bucket_mask_);
    ...
    static const char * name() {
        ...
        case HashFunc_CRC32C:
            return "KVCache::Dictionary<K, V>";
        case HashFunc_Time31:
            return "KVCache::Dictionary_v1<K, V>";
        case HashFunc_Time31Std:
            return "KVCache::Dictionary_v2<K, V>";
        case HashFunc_SHA1_MSG2:
            return "KVCache::Dictionary_v3<K, V>";
        case HashFunc_SHA1:
            return "KVCache::Dictionary_v4<K, V>";
        default:
...
#if SUPPORT_SMID_SHA
template <typename Key, typename Value>
using Dictionary_SHA_v1 = BasicDictionary<Key, Value, HashFunc_SHA1_MSG2>;
#endif

#if SUPPORT_SMID_SHA
template <typename Key, typename Value>
using Dictionary_SHA_v2 = BasicDictionary<Key, Value, HashFunc_SHA1>;
#endif
